...
# Allow both interactive (manual) and app-driven rendering
if (!exists("params") || is.null(params$params_file)) {
  message("Running interactively — setting default params file manually")
  params <- list(
...
tagList(plotly::as_widget(combined_plot))
```
...
if (bedToBigBed_path == "") {
  # Check HiPerGator path if not found on PATH
  alt_path <- "/apps/ucsc/20210803/bedToBigBed"
  if (file.exists(alt_path)) {
    bedToBigBed_path <- alt_path
  } else {
    stop("bedToBigBed not found on PATH or at /apps/ucsc/20210803/bedToBigBed")
  }
}
...
trackhub_base_url <- "http://data.rc.ufl.edu/pub/cancercenter-dept/BCB-SR"
